...
class ProfileAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Ensure the logged-in user doesn't already have a profile
        if Profile.objects.filter(user=request.user).exists():  # Change: Check if profile exists for user
...
class ProfileDetailAPIView(APIView):
    ...
    def get_object(self):
        ...
        profile = get_object_or_404(Profile, user=self.request.user)  # Modified to fetch profile by user
        return profile
